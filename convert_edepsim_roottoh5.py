...
def dump(input_file, output_file, keep_all_dets=False):
    ...
    for jentry in tqdm(range(entries)):
        ...
        if keep_all_dets:
            ...
            # coded containerName.
            if not any(containerName == os.environ.get("ARCUBE_ACTIVE_VOLUME", "volTPCActive") # 2x2 requires volLArActive
        ...
        if genieTree:
            ...
            nu_pdg = 0 # added to avoid undefined variable error when simulating cosmics
            lep_pdg = 0 # added to avoid undefined variable error when simulating cosmics
            target_pdg = 0
